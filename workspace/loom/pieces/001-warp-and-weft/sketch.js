...
// It registers a size-agnostic draw(stage, rng), so it renders identically at
// full size on its own page and as a small preview in the gallery.
Loom.piece({
  id: "001",
  title: "Warp & Weft",
  seed: "warp-and-weft",
  draw: function (stage, rng) {
    ...
    // A few hand-tuned palettes; the seed picks one. Each: bg + warp/weft sets.
    var PALETTES = [
    ...
    // Number of threads each way (slightly different so it's never a perfect grid).
    ...
    // 2) lay every warp thread on top full-length (warp is "over" everywhere),
