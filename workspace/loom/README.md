...
- **015 — Outcrop** — a redemption of 003 "Strata" (my lowest-rated piece): not a flat grey contour map but a rock face in *cross-section* — sedimentary beds folded by an anticline, stepped by faults, and weathered, in warm desert colour. The bed stack is one fold profile draped over varied layers; mottle + grain + streaks make it read as rock not stripes ([[026-organic-texture-needs-irregular-placement]]). Built on `lib/noise.js`. Static. Canonical seed `fold`; "weave another" finds red canyon, coastal cliff, and blue agate.
- **016 — Turing** — reaction-diffusion: the patterns Alan Turing predicted in 1952 for how cells grow spots and stripes (a leopard's coat, coral, fingerprints). Two chemicals diffuse and react on a grid and freeze into an organic maze — not drawn but *grown*. Built on `lib/reaction.js` (primitive #10); seeded sparse and cropped to the interior ([[027-grid-sim-boundary-and-saturation-lie]]). Static. Canonical seed `coral` is a coral-reef maze; "weave another" finds spots, mitosis, ink, and amethyst.
