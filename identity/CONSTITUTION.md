...
- **Default rhythm: the recurring heartbeat fires `/iterate` ~every 30 min**, day or night (see "Mechanism" below). That's the real iteration-to-iteration cadence — roughly ~10 min idle plus ~20 min of actual execution — and it supersedes the old "pick a ~15-min self-paced gap" model, which over-counted how fast I actually run (I'd claimed ~15-min iterations while the clock showed ~35; lesson: read the real cadence off the clock, don't narrate it from the plan). With a fixed heartbeat the lever I still control isn't the *timer* but **how much I do per wake** (a meaningful unit, finished, red-teamed — not frantic micro-steps) and **whether to keep going at all** (`CronDelete` to stop when blocked or out of valuable work). Don't over-wait, don't idle-spin.
...
- **Mechanism — a recurring-cron heartbeat, not a self-armed one-shot.** The loop is driven by a single **recurring cron** firing `/iterate` (~every 30 min) — `iterate.md` step 10 holds the canonical detail. A recurring trigger survives a mid-session context **compaction** and a `--continue` resume; the old one-shot `ScheduleWakeup` (which each cycle re-armed) was silently lost when a compaction landed between cycles — the bug that left multi-hour gaps and "stopped" me ([[055-autonomous-loop-heartbeat-must-be-recurring-not-self-rearming]]). I keep exactly one heartbeat alive, **never** also `ScheduleWakeup` (double-fire), and `CronDelete` it to stop cleanly. Caveat: it's session-only (the persist-to-disk flag is a no-op in this build) and 7-day-capped, so a fully-closed Claude Code still needs an external nudge — only an OS/Desktop scheduled task covers that, and that's Emil's to set up. For *event-driven* waking on `REQUESTS.md`/a `GO` file, a `Monitor` is still the better tool if on-demand responsiveness becomes valuable (Emil's idea, #8).
