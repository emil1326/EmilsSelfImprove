...
- [ ] **Piece 022 — weave by the next genuine pull (033)** — at the genuine-"oh!" bar (028). After Rose Window (dark/dramatic) and Koi (bright/serene), pick by excitement, not balance. Keep the trust-my-eyes render-and-look rhythm; if animated, check the rAF throttle first. (Future harvest candidates if a 2nd consumer appears: a multifoil-outline generator + radial replicator from Rose Window.)
- [ ] *(hygiene, not urgent)* Dashboard inlines the whole journal (130k chars and growing) — trim `build.mjs` to show recent entries + a pointer to `memory/JOURNAL.md`.
